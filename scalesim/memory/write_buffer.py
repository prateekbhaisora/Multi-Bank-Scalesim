...
class write_buffer:
    ...
    def service_writes(self, incoming_requests_arr_np, incoming_cycles_arr_np):
        # Checking if input data is consistent
        ...
        '''
        For each request:
            Retrieves the row (request) and the corresponding cycle from the input arrays.
            Calculates the current cycle by adding the cycle offset (offset) to the input cycle.
            Checks if the current cycle is before the drain end cycle. 
                If so, it ensures that there is free space in the OFMAP buffer for writing. 
                If not, it updates the offset to skip cycles until the drain end cycle.
        If there is sufficient free space in the OFMAP buffer, it stores the data to the trace matrix cache.
        Checks if the free space in the OFMAP buffer is below a certain threshold (self.total_size_elems - self.drain_buf_size). 
            If so, it appends the data to the trace matrix and empties the drain buffer to create more space for future writes.
        This ensures that the trace matrix accurately reflects the writes to the OFMAP buffer.
        After processing all requests, the function constructs an output array containing the cycle at which each request is completed and returns it.
        '''

        for i in tqdm(range(incoming_requests_arr_np.shape[0]), disable=True):
            row = incoming_requests_arr_np[i]       # Retrieve current request
            cycle = incoming_cycles_arr_np[i]       # Retrieve corresponding cycle
            current_cycle = cycle[0] + offset       # Calculate current cycle
            ...
            for elem in row:
                ...
                # The "drain end cycle" refers to the cycle at which the drain buffer (a portion of the OFMAP buffer) 
                # is emptied to create more space for future write operations.
                if current_cycle < self.drain_end_cycle:
                    if not self.free_space > 0:
                        # Skip cycles until drain end cycle is reached
